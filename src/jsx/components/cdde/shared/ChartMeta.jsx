...
function linkifySource(text, unctadLink) {
  ...
  const pairs = SOURCE_LINKS.map(([label, href]) => [label, label === 'UNCTADstat' && unctadLink?.includes('unctadstat') ? unctadLink : href]).filter(([label]) => text.includes(label));
  if (!pairs.length) return text;
  const pattern = new RegExp(`(${pairs.map(([l]) => l.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})`);
  const linkMap = Object.fromEntries(pairs);
  return text.split(pattern).map((part, _i) =>
    linkMap[part] ? (
      <a key={part} href={linkMap[part]} target="_blank" rel="noopener">
        {part}
      </a>
    ) : (
      part
