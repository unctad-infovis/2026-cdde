...
export default function SocialContext({ iso3, title, subtitle, description, source, note }) {
  ...
  const items = d ? [
    { label: 'Employment rate', value: d.emp != null ? `${d.emp}%` : '–', note: '2024, working-age population' },
    { label: 'Female employment', value: d.emp_female != null ? `${d.emp_female}%` : '–', note: '2024, working-age population' },
    { label: 'Population', value: fmtPop(d.population), note: '2024 estimate' },
    {
      label: 'Human Development Index',
      value: d.hdi_value != null ? d.hdi_value.toFixed(3) : '–',
      note: d.hdi_rank != null ? `Rank ${d.hdi_rank}${d.hdi_category ? ` – ${d.hdi_category}` : ''}` : undefined,
    },
  ] : [];
