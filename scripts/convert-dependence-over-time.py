...
wb = openpyxl.load_workbook('data/CommodityDependency.xlsx', data_only=True)
ws = wb.active
...
for row in ws.iter_rows(min_row=2, values_only=True):
    ...
    for col_idx, year in year_cols:
        ...
        if val is not None and isinstance(val, (int, float)):
            ...
            series.append({'year': year, 'pct': pct})
    if series:
        out[iso3] = series

out_path = 'public/assets/data/cdde_dependence_over_time.json'
